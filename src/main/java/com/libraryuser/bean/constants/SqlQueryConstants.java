...
public interface SqlQueryConstants {
	...
	public static final String INSERT_USER_STATEMENT = "INSERT INTO user(name,email,active,role_id,update_timestamp) "
			+ "values(?,?,?,?,?)";
	public static final String INSERT_PASSWORD_STATEMENT = "INSERT INTO password(user_id,password)values(?,?)";
	public static final String DEACTIVATE_PASSWORD_STATEMENT = "UPDATE password SET active = ? WHERE user_id = ? AND active = ?";
